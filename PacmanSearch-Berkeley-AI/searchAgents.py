...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        if node_xy in self.corners:
            if not node_xy in visitedCorners:
                visitedCorners.append(node_xy)
...
def cornersHeuristic(state, problem):
    ...
    # We find which corners are left to reach GoalState.
    visitedCorners = state[1]
    notVisitedCorners = []
    ...
    # While not all corners are visited, find the most efficient path for each corner via manhattanDistance
    while notVisitedCorners:
        heuristic_cost, corner = \
            min([(util.manhattanDistance(currentPoint, corner), corner) for corner in notVisitedCorners])
...
def mazeDistance(p1, p2, gameState):
    """
    ...
    prob = PositionSearchProblem(gameState, start=p1, goal=p2, warn=False, visualize=False)
